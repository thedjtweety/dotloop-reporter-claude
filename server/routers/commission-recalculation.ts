...
export const commissionRecalculationRouter = router({
   ...
   * Returns array of agent commission data
   */
  getAllAgentCommissionSummaries: publicProcedure
    .input(z.object({
      tenantId: z.number().optional(),
    }))
    .query(async (opts: any) => {
      ...
      try {
        ...
        // Get commission summary for each agent
        for (const assignment of allAssignments) {
          ...
          const relevantTransactions = agentTransactions.filter(t => {
            if (!t.agents) return false;
            const agentsList = t.agents.split(',').map(a => a.trim());
            return agentsList.some(a => a.toLowerCase() === assignment.agentName.toLowerCase());
          });
              ...
              const breakdown = calculateTransactionCommission(
                {
                  ...
                  loopName: transaction.loopName || 'Unknown',
                  closingDate: transaction.closingDate || transaction.createdDate || new Date().toISOString(),
                  agents: transaction.agents || assignment.agentName,
                  salePrice: transaction.salePrice || transaction.price || 0,
                  commissionRate: transaction.commissionRate || 0,
                },
        ...
        console.error('[commissionRecalculation.getAllAgentCommissionSummaries]', error);
        return {
          summaries: [],
          error: error instanceof Error ? error.message : 'Unknown error',
        };
  ...
  recalculateAll: publicProcedure
    ...
    .mutation(async (opts: any) => {
      ...
      try {
        ...
        console.log(`[commissionRecalculation.recalculateAll] Starting bulk recalculation`);
        ...
        for (const assignment of allAssignments) {
          ...
          for (const transaction of sortedTransactions) {
            try {
              ...
              agentCommission += breakdown.agentNetCommission;
              ytdCompanyDollar = breakdown.ytdAfterTransaction;
            } catch (error) {
              console.error(`Error calculating commission for transaction:`, error);
            }
        ...
        console.log(`[commissionRecalculation.recalculateAll] Completed: ${totalAgents} agents, ${totalTransactions} transactions`);
        ...
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: error instanceof Error ? error.message : 'Failed to recalculate all commissions',
        });
      }
    }),
});
