# Terms of Service

**Effective Date:** February 20, 2026  
**Last Updated:** February 20, 2026

## Agreement to Terms

These Terms of Service ("Terms") constitute a legally binding agreement between you ("User," "you," or "your") and Dotloop Reporter ("Company," "we," "our," or "us"), an independent passion project, governing your access to and use of the Dotloop Reporter platform (the "Service"). We are not affiliated with or endorsed by Dotloop.

By accessing or using the Service, you agree to be bound by these Terms. If you do not agree to these Terms, you may not access or use the Service.

## Eligibility

To use the Service, you must:
- Be at least 18 years of age
- Have the legal capacity to enter into binding contracts
- Be a licensed real estate professional or authorized employee of a real estate brokerage
- Not be prohibited from using the Service under applicable laws

By using the Service, you represent and warrant that you meet these eligibility requirements.

## Account Registration and Security

### Account Creation

To access certain features, you must create an account by:
- Providing accurate and complete registration information
- Connecting your Dotloop account via OAuth 2.0 authentication
- Accepting these Terms and our Privacy Policy

### Account Security

You are responsible for:
- Maintaining the confidentiality of your account credentials
- All activities that occur under your account
- Notifying us immediately of any unauthorized access or security breaches
- Using strong passwords and enabling available security features

We reserve the right to suspend or terminate accounts that violate security policies or these Terms.

### Account Types and Roles

The Service supports multiple user roles with different permissions:
- **Admin:** Full system access, user management, audit log viewing
- **Broker:** Team data access, commission plan management, agent oversight
- **Agent:** Personal and team data access, upload capabilities
- **Viewer:** Read-only access to assigned data

Your brokerage administrator assigns your role and permissions.

## Acceptable Use

### Permitted Uses

You may use the Service to:
- Import and analyze real estate transaction data
- Generate commission reports and financial analytics
- Manage commission plans and agent assignments
- Export reports for business purposes
- Collaborate with team members within your brokerage

### Prohibited Uses

You may NOT:
- Use the Service for any illegal or unauthorized purpose
- Violate any applicable laws, regulations, or third-party rights
- Upload malicious code, viruses, or harmful content
- Attempt to gain unauthorized access to the Service or other users' data
- Scrape, crawl, or use automated tools to extract data beyond normal use
- Reverse engineer, decompile, or disassemble the Service
- Resell, sublicense, or redistribute the Service without authorization
- Impersonate another user or provide false information
- Interfere with or disrupt the Service or servers
- Use the Service to spam, harass, or harm others

Violation of these prohibitions may result in immediate account termination and legal action.

## Data and Content

### Your Data

You retain ownership of all data you upload to the Service ("Your Data"), including:
- Transaction records and property information
- Agent data and commission structures
- Custom fields and imported CSV files
- Reports and exports you generate

### License to Your Data

By uploading Your Data, you grant us a limited, non-exclusive license to:
- Store, process, and display Your Data to provide the Service
- Perform calculations and generate analytics based on Your Data
- Create backups and ensure data redundancy
- Comply with legal obligations

This license terminates when you delete Your Data or close your account.

### Your Responsibilities

You are solely responsible for:
- The accuracy, legality, and quality of Your Data
- Ensuring you have the right to upload and share Your Data
- Compliance with data protection laws (GDPR, CCPA, etc.)
- Obtaining necessary consents from individuals whose data you process
- Maintaining backups of Your Data

We are not responsible for loss or corruption of Your Data, though we implement reasonable safeguards.

### Prohibited Content

You may not upload data that:
- Violates laws or regulations
- Infringes intellectual property rights
- Contains malware or malicious code
- Includes personal information of individuals under 18
- Violates privacy rights or confidentiality obligations

## Dotloop Integration

### OAuth Connection

The Service connects to your Dotloop account using OAuth 2.0 authentication. By connecting:
- You authorize us to access your Dotloop transaction data
- You agree to Dotloop's Terms of Service and Privacy Policy
- You understand that we are not affiliated with or endorsed by Dotloop
- You acknowledge that Dotloop may revoke access at any time

### Data Synchronization

When connected to Dotloop:
- We periodically sync your transaction data based on your settings
- You can configure sync frequency and data filters
- You can disconnect your Dotloop account at any time
- Disconnecting stops future syncs but does not delete previously imported data

### Third-Party Service

Dotloop is a third-party service. We are not responsible for:
- Dotloop's availability, performance, or data accuracy
- Changes to Dotloop's API or policies
- Dotloop's handling of your data
- Disputes between you and Dotloop

## Privacy and Data Protection

### Privacy Policy

Our collection and use of personal information is governed by our Privacy Policy, which is incorporated into these Terms by reference. Please review the Privacy Policy to understand our data practices.

### Data Security

We implement industry-standard security measures, including:
- Encryption in transit (TLS 1.2+) and at rest (AES-256)
- Role-based access control (RBAC)
- Audit logging and monitoring
- Regular security assessments

However, no system is completely secure. You use the Service at your own risk.

### Data Breach Notification

In the event of a data breach affecting your personal information, we will:
- Notify you within 72 hours of discovery
- Provide details about the breach and affected data
- Describe steps we are taking to address the breach
- Offer guidance on protecting yourself

### Compliance

You agree to comply with all applicable data protection laws, including:
- General Data Protection Regulation (GDPR)
- California Consumer Privacy Act (CCPA)
- State and federal privacy laws
- Real estate industry regulations

## Intellectual Property

### Our Intellectual Property

The Service and all related content, features, and functionality are owned by us and protected by:
- Copyright, trademark, and patent laws
- Trade secret and other intellectual property rights

You may not copy, modify, distribute, or create derivative works without our express written permission.

### Trademarks

"Dotloop Reporter" and related logos are our trademarks. You may not use our trademarks without prior written consent.

### Feedback

If you provide feedback, suggestions, or ideas about the Service, you grant us a perpetual, irrevocable, royalty-free license to use, implement, and commercialize such feedback without compensation or attribution.

## Payment and Billing

### Subscription Plans

The Service may offer various subscription plans with different features and pricing. By subscribing:
- You agree to pay all applicable fees
- Fees are billed in advance on a recurring basis
- All fees are non-refundable except as required by law
- We may change pricing with 30 days' notice

### Payment Methods

We accept payment via credit card, debit card, or other approved methods. You authorize us to:
- Charge your payment method for all fees
- Update payment information automatically (if supported)
- Suspend access for non-payment

### Taxes

You are responsible for all taxes, duties, and assessments associated with your use of the Service, except for taxes based on our net income.

## Term and Termination

### Term

These Terms begin when you first access the Service and continue until terminated by either party.

### Termination by You

You may terminate your account at any time by:
- Using the account deletion feature in settings
- Contacting our support team
- Ceasing all use of the Service

### Termination by Us

We may suspend or terminate your account immediately if:
- You violate these Terms or our policies
- You engage in fraudulent or illegal activity
- Your account poses a security risk
- You fail to pay applicable fees
- We discontinue the Service

### Effect of Termination

Upon termination:
- Your right to access the Service immediately ceases
- We may delete Your Data after 30 days
- You remain liable for all fees incurred before termination
- Provisions that should survive termination will continue (e.g., indemnification, limitations of liability)

You may export Your Data before termination using our data export features.

## Disclaimers and Limitations of Liability

### Disclaimer of Warranties

THE SERVICE IS PROVIDED "AS IS" AND "AS AVAILABLE" WITHOUT WARRANTIES OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO:
- MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, OR NON-INFRINGEMENT
- ACCURACY, RELIABILITY, OR COMPLETENESS OF DATA
- UNINTERRUPTED OR ERROR-FREE OPERATION
- SECURITY OR FREEDOM FROM VIRUSES

WE DO NOT WARRANT THAT:
- The Service will meet your requirements
- Data will be accurate or up-to-date
- Errors will be corrected
- The Service will be available at all times

### Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY LAW, WE SHALL NOT BE LIABLE FOR:
- INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES
- LOSS OF PROFITS, REVENUE, DATA, OR BUSINESS OPPORTUNITIES
- COST OF SUBSTITUTE SERVICES
- DAMAGES RESULTING FROM YOUR USE OR INABILITY TO USE THE SERVICE

OUR TOTAL LIABILITY SHALL NOT EXCEED THE GREATER OF:
- $100 USD, OR
- The amount you paid us in the 12 months preceding the claim

Some jurisdictions do not allow limitation of liability, so these limitations may not apply to you.

### Indemnification

You agree to indemnify, defend, and hold harmless Dotloop Reporter, its officers, directors, employees, and agents from any claims, damages, losses, liabilities, and expenses (including attorneys' fees) arising from:
- Your use of the Service
- Your violation of these Terms
- Your violation of any rights of another party
- Your Data or content you upload

## Dispute Resolution

### Governing Law

These Terms are governed by the laws of the State of Delaware, United States, without regard to conflict of law principles.

### Arbitration Agreement

Any dispute arising from these Terms or the Service shall be resolved through binding arbitration administered by the American Arbitration Association (AAA) under its Commercial Arbitration Rules.

You agree to:
- Arbitrate disputes on an individual basis (no class actions)
- Waive your right to a jury trial
- Waive your right to participate in class action lawsuits

### Exceptions to Arbitration

The following may be brought in court:
- Small claims court actions (up to jurisdictional limit)
- Injunctive or equitable relief for intellectual property infringement
- Disputes that cannot be arbitrated by law

### Informal Resolution

Before initiating arbitration, you agree to attempt informal resolution by contacting us at dotloopreport@gmail.com.

## General Provisions

### Entire Agreement

These Terms, together with our Privacy Policy, constitute the entire agreement between you and us regarding the Service.

### Amendments

We may modify these Terms at any time by:
- Posting updated Terms on our website
- Notifying you via email or in-app notification
- Providing 30 days' notice for material changes

Your continued use after changes become effective constitutes acceptance.

### Severability

If any provision of these Terms is found invalid or unenforceable, the remaining provisions will remain in full force and effect.

### Waiver

Our failure to enforce any provision does not waive our right to enforce it later.

### Assignment

You may not assign or transfer these Terms without our written consent. We may assign these Terms to any successor or affiliate.

### Force Majeure

We are not liable for delays or failures caused by circumstances beyond our reasonable control, including natural disasters, war, terrorism, labor disputes, or internet outages.

### Notices

Notices to you may be sent via email to your registered email address or through in-app notifications. Notices to us should be sent to dotloopreport@gmail.com.

### Export Control

You agree to comply with all export and import laws and regulations. You represent that you are not located in a country subject to U.S. embargo or designated as a "terrorist supporting" country.

## Contact Information

For questions about these Terms, please contact us:

**Email:** dotloopreport@gmail.com  
**Project:** Independent open-source passion project  
**Support:** Available via email

---

*These Terms of Service should be reviewed by legal counsel before deployment to ensure compliance with applicable laws and regulations.*
