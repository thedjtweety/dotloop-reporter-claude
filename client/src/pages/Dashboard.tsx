...
function DrillDownModal({ target, onClose }: { target: DrillTarget | null; onClose: () => void }) {
  return (
    <Dialog open={!!target} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl bg-[#0d1117] border border-[#1e2d3d] text-white">
        <DialogHeader>
          <DialogTitle className="text-white">{target?.title}</DialogTitle>
        </DialogHeader>
        <ScrollArea className="max-h-[60vh]">
          {(target?.records.length ?? 0) === 0 ? (
            <p className="text-gray-400 py-8 text-center text-sm">No transactions in this category.</p>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody>
                {target!.records.slice(0, 50).map((r, i) => (
                  <tr key={i} className="border-b border-[#1e2d3d]/40 hover:bg-[#111827]">
                    <td className="py-2 pr-4 text-white">{r.address || r.loopName || '—'}</td>
                    <td className="py-2 pr-4 text-gray-300 text-xs">{r.agents || '—'}</td>
                    <td className="py-2 pr-4">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                        r.loopStatus === 'Closed'
                          ? 'bg-emerald-500/20 text-emerald-400'
                          : r.loopStatus === 'Under Contract'
                    ...
                    <td className="py-2 pr-4 text-right text-gray-300 text-xs">
                      {r.salePrice ? compactCurrency(r.salePrice) : '—'}
                    </td>
                    <td className="py-2 text-right text-emerald-400 text-xs">
        ...
        {(target?.records.length ?? 0) > 50 && (
          <p className="text-gray-500 text-xs mt-2">Showing 50 of {target!.records.length} records.</p>
        )}
      </DialogContent>
    </Dialog>
  );
...
function SectionLabel({ children, sub }: { children: React.ReactNode; sub?: string }) {
  return (
    <div className="flex items-baseline gap-3 mb-3">
      <h2 className="text-white font-semibold text-xs tracking-widest uppercase">{children}</h2>
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 px-1 pb-8">
      ...
      <section>
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1">
          {visibleActions.map(item => (
            <div
              key={item.id}
              className="flex-none w-60 bg-[#0d1117] border border-[#1e2d3d] rounded-xl p-4 cursor-pointer hover:border-[#2a3f55] transition-colors group"
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    className="text-gray-700 hover:text-gray-400 ml-0.5 opacity-0 group-hover:opacity-100 transition-opacity"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ...
              <p className="text-gray-600 text-xs">{item.agent}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-[#0d1117] border border-[#1e2d3d] rounded-xl p-5">
        ...
        <div className="mb-5">
          <div className="flex h-4 rounded-full overflow-hidden bg-[#1a2332]">
            ...
            {gapPct > 0 && <div className="h-full flex-1 bg-[#1a2332]" style={{ backgroundImage: 'repeating-linear-gradient(90deg, transparent, transparent 4px, #ef444433 4px, #ef444433 8px)' }} />}
          </div>
        ...
        <div className="flex gap-3 flex-wrap">
          {[
            ...
            <div key={f.label} className="flex-1 min-w-[100px] bg-[#111827] rounded-lg px-4 py-3 text-center">
              <p className={`text-2xl font-bold tabular-nums ${f.color}`}>{f.count}</p>
              <p className="text-white text-xs font-medium mt-0.5">{f.label}</p>
              <p className="text-gray-500 text-xs mt-0.5">{compactCurrency(f.volume)}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div
            ...
            className={`bg-[#0d1117] border ${card.border} rounded-xl p-5 cursor-pointer hover:bg-[#111827] transition-colors`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            {pipelineBreakdown.map(item => (
              <div
                ...
                className="flex items-center justify-between border-l-4 pl-3 py-2.5 rounded-r-lg hover:bg-[#111827] cursor-pointer transition-colors"
                style={{ borderLeftColor: item.color }}
              >
                <div>
                  <p className="text-white text-sm font-medium">{item.label}</p>
                  <p className="text-gray-500 text-xs">{item.volume > 0 ? compactCurrency(item.volume) : '—'}</p>
                </div>
                <div className="text-right">
                  <p className="text-white font-bold tabular-nums">{item.count}</p>
                  <p className="text-gray-500 text-xs">{((item.count / Math.max(totalTx, 1)) * 100).toFixed(0)}%</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-[#0d1117] border border-[#1e2d3d] rounded-xl p-5">
          ...
          <Tabs value={projectedTab} onValueChange={v => setProjectedTab(v as '30' | '60' | '90')}>
            <TabsList className="bg-[#1a2332] mb-4 h-8">
              {(['30', '60', '90'] as const).map(t => (
                <TabsTrigger key={t} value={t} className="data-[state=active]:bg-[#1e3a5f] data-[state=active]:text-white text-gray-400 text-xs h-full">
                  {t}d
                </TabsTrigger>
              ))}
            </TabsList>
              ...
              <TabsContent key={t} value={t}>
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={m.label} className="bg-[#111827] rounded-lg p-3 text-center">
                      <p className={`text-xl font-bold tabular-nums ${m.color}`}>{m.value}</p>
                      <p className="text-gray-400 text-xs mt-1 leading-tight">{m.label}</p>
                    </div>
                  ))}
                </div>
              </TabsContent>
          ...
          <div className="mt-5">
            <div className="flex items-center justify-between mb-2">
              <p className="text-gray-400 text-xs">Commission Risk Adjustment</p>
              <p className="text-white text-xs font-semibold tabular-nums">{riskAdjust[0]}%</p>
            </div>
      ...
      <section className="bg-[#0d1117] border border-[#1e2d3d] rounded-xl p-5">
        ...
        <div className="grid grid-cols-7 gap-2">
          {weekDays.map(day => (
            ...
            >
              <p className={`text-xs font-semibold mb-2 ${day.isToday ? 'text-blue-400' : 'text-gray-500'}`}>
                {day.label}
              </p>
              {/* Mini bar */}
              <div className="flex justify-center items-end h-9 mb-2">
                <div
                  className={`w-5 rounded-t-sm transition-all ${day.isToday ? 'bg-blue-500' : 'bg-[#1e3a5f]'}`}
                  style={{ height: `${Math.max((day.count / maxDayCount) * 100, 8)}%` }}
                />
              </div>
              <p className={`text-xl font-bold tabular-nums ${day.isToday ? 'text-blue-300' : 'text-white'}`}>
                {day.count}
              </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div key={item.label} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs flex items-center ${
                    item.trend > 0 ? 'text-emerald-400' : item.trend < 0 ? 'text-red-400' : 'text-gray-600'
                  }`}>
                    {item.trend > 0
            ...
            {RECRUITING_STAGES.map(stage => (
              <div key={stage.label}>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-white font-bold text-sm tabular-nums">{stage.count}</span>
                    <span className="text-gray-600 text-xs tabular-nums w-8 text-right">{stage.pct}%</span>
                  </div>
                </div>
        ...
        <div className="bg-[#0d1117] border border-[#1e2d3d] rounded-xl p-5">
          ...
          <div className="flex items-center gap-4 mb-5">
            ...
            <div>
              <p className="text-white font-bold text-2xl tabular-nums">92%</p>
              <p className="text-gray-400 text-xs">Compliance score</p>
            ...
            {COMPLIANCE_ISSUES.map(issue => (
              <div key={issue.label} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-300 text-xs">{issue.label}</span>
                </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-3 gap-4">
          {MARKET_PULSE.map(item => (
            <div key={item.label} className="bg-[#0d1117] border border-[#1e2d3d] rounded-xl p-5">
              <p className="text-gray-500 text-xs mb-2">{item.label}</p>
              <p className="text-white text-2xl font-bold tabular-nums mb-1">{item.value}</p>
              <p className="text-gray-600 text-xs mb-2">{item.sub}</p>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {aiInsights.map((insight, i) => (
            <div key={i} className="bg-[#0d1117] border border-[#1e2d3d] rounded-xl p-5">
              ...
              <p className="text-white text-sm font-semibold mb-1.5 leading-snug">{insight.title}</p>
